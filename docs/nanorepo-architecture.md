...
2. **Gemini Code** — reads `SKILL.md`, `.intent.md`, and `state.yaml` to resolve conflicts git can't handle. Caches resolutions via `git rerere` so the same conflict never needs resolving twice.
3. **Gemini Code + user input** — when Gemini Code lacks sufficient context to determine intent (e.g., two features genuinely conflict at an application level), it asks the user for a decision, then uses that input to perform the resolution. Gemini Code still does the work — the user provides direction, not code.
...
### Intent Files
Each modified file has a `.intent.md` with structured headings: **What this skill adds**, **Key sections**, **Invariants**, and **Must-keep sections**. These give Gemini Code specific guidance during conflict resolution.
...
6. Conflict resolution (shared cache → `git rerere` → Gemini Code → Gemini Code + user input)
...
2. **Three-level resolution: git → Gemini Code → Gemini Code + user input.**
