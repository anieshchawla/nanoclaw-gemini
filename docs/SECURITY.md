...
**Mounted Credentials:**
- Gemini auth tokens (filtered from `.env`, read-only)
...
```typescript
const allowedVars = ['GEMINI_OAUTH_TOKEN', 'GEMINI_API_KEY'];
```

> **Note:** Google credentials are mounted so that Gemini Code can authenticate when the agent runs. However, this means the agent itself can discover these credentials via Bash or file operations. Ideally, Gemini Code would authenticate without exposing credentials to the agent's execution environment, but I couldn't figure this out. **PRs welcome** if you have ideas for credential isolation.
