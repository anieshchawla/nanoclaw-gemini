# Gemini Agent SDK Deep Dive

Findings from reverse-engineering `@google/gemini-cli-core` v0.2.29–0.2.34 to understand how `query()` works, why agent teams subagents were being killed, and how to fix it. Supplemented with official SDK reference docs.
...
Agent Runner (our code)
  └── query() → SDK (sdk.mjs)
        └── spawns CLI subprocess (cli.js)
              └── Gemini API calls, tool execution
...
Each invocation = one API call to Gemini (one "turn").

### Flow per turn:

1. **Prepare messages** — trim context, run compaction if needed
2. **Call the Google API** (via `mW1` streaming function)
...
| `additionalDirectories` | `string[]` | `[]` | Additional directories Gemini can access |
...
| `model` | `string` | Default from CLI | Gemini model to use |
| `outputFormat` | `{ type: 'json_schema', schema: JSONSchema }` | `undefined` | Structured output format |
| `pathToGeminiCodeExecutable` | `string` | Uses built-in | Path to Gemini Code executable |
...
| `settingSources` | `SettingSource[]` | `[]` (none) | Which filesystem settings to load. Must include `'project'` to load GEMINI.md |
| `stderr` | `(data: string) => void` | `undefined` | Callback for stderr output |
| `systemPrompt` | `string \| { type: 'preset'; preset: 'gemini_code'; append?: string }` | `undefined` | System prompt. Use preset to get Gemini Code's prompt, with optional `append` |
| `tools` | `string[] \| { type: 'preset'; preset: 'gemini_code' }` | `undefined` | Tool configuration |
...
// 'project' → .gemini/settings.json (version controlled)
// 'local'   → .gemini/settings.local.json (gitignored)
```
...
| `assistant` | — | Gemini's response (text + tool calls) |
...
type SDKAssistantMessage = {
  ...
  uuid: UUID;
  session_id: string;
  message: APIAssistantMessage; // From Google SDK
...
Gemini responded with text only — it decided it has completed the task. The API's `stop_reason` will be `"end_turn"`. The SDK does NOT make this decision — it's entirely driven by Gemini's model output.
...
2. assistant/user       → Gemini reasoning, tool calls, tool results
...
| `mW1` | Google API streaming caller |
