...
export async function run(args: string[]): Promise<void> {
  ...
  // Update assistant name in GEMINI.md files if different from default
  let nameUpdated = false;
  if (parsed.assistantName !== 'Andy') {
    logger.info({ from: 'Andy', to: parsed.assistantName }, 'Updating assistant name');

    const mdFiles = [
      path.join(projectRoot, 'groups', 'global', 'GEMINI.md'),
      path.join(projectRoot, 'groups', 'main', 'GEMINI.md'),
    ];

    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        let content = fs.readFileSync(mdFile, 'utf-8');
        content = content.replace(/^# Gemini$/m, `# ${parsed.assistantName}`);
        content = content.replace(/You are Gemini/g, `You are ${parsed.assistantName}`);
        fs.writeFileSync(mdFile, content);
        logger.info({ file: mdFile }, 'Updated GEMINI.md');
      }
    }
