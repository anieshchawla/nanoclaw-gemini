...
const shippedResDir = path.join(projectRoot, '.gemini', 'resolutions', 'discord+telegram');
...
function getRrCacheEntries(): Set<string> {
  if (!fs.existsSync(rrCacheDir)) return new Set<string>();
...
for (const relPath of conflictFiles) {
  ...
  const resolutionContent = (() => {
    ...
    const backupPath = path.join(projectRoot, '.gemini', 'resolutions', '_backup', relPath + '.resolution');
