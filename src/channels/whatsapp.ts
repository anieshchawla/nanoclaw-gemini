...
export class WhatsAppChannel implements Channel {
  ...
  private async connectInternal(onFirstOpen?: () => void): Promise<void> {
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (qr) {
        const msg =
          'WhatsApp authentication required. Run /setup in Gemini.';
        logger.error(msg);
        exec(
    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      logger.debug({ count: messages.length }, 'Messages upserted');
      for (const msg of messages) {
        if (!msg.message) {
          logger.debug({ key: msg.key }, 'Upserted message has no content');
          continue;
        }
        ...
        logger.info({ rawJid, chatJid, content: !!msg.message?.conversation, keys: Object.keys(msg.message) }, 'Incoming message');
        ...
        if (groups[chatJid]) {
          ...
          const assistantName = group?.trigger?.startsWith('@')
