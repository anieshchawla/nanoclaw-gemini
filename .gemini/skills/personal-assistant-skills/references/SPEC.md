...
| Agent | @google/gemini-cli-core (0.2.29) | Run Gemini with tools and MCP servers |
...
```
nanoclaw/
├── GEMINI.md                      # Project context for Gemini Code
├── docs/
...
│
├── container/
│   ├── Dockerfile                 # Container image (runs as 'node' user, includes Gemini Code CLI)
...
│   ├── sessions/                  # Per-group session data (.gemini/ dirs with JSONL transcripts)
...
**Option 1: Gemini Subscription (OAuth token)**
```bash
GEMINI_OAUTH_TOKEN=sk-ant-oat01-...
```
The token can be extracted from `~/.gemini/.credentials.json` if you're logged in to Gemini Code.
...
Only the authentication variables (`GEMINI_OAUTH_TOKEN` and `GEMINI_API_KEY`) are extracted from `.env` and written to `data/env/env`, then mounted into the container at `/workspace/env-dir/env` and sourced by the entrypoint script. This ensures other environment variables in `.env` are not exposed to the agent. This workaround is needed because some container runtimes lose `-e` environment variables when using `-i` (interactive mode with piped stdin).
...
1. **Agent Context Loading**
   ...
   - Gemini Agent SDK with `settingSources: ['project']` automatically loads:
...
2. Session ID is passed to Gemini Agent SDK's `resume` option
3. Gemini continues the conversation with full context
4. Session transcripts are stored as JSONL files in `data/sessions/{group}/.gemini/`
...
6. Router catches up conversation:
   ...
   │
   ▼
7. Router invokes Gemini Agent SDK:
...
Gemini: [calls mcp__nanoclaw__schedule_task]
        {
...
|------------|------------------|-------|
| Gemini CLI Auth | data/sessions/{group}/.gemini/ | Per-group isolation, mounted to /home/node/.gemini/ |
...
| "Gemini Code process exited with code 1" | Container runtime failed to start | Check logs; NanoClaw auto-starts container runtime but may fail |
| "Gemini Code process exited with code 1" | Session mount path wrong | Ensure mount is to `/home/node/.gemini/` not `/root/.gemini/` |
